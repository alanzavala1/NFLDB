...
function PlayerNode({ player, mirror, onSelect }: { player: Placed; mirror: boolean; onSelect: (p: LineupPlayer) => void }) {
  ...
  // Skill players need a few graded plays before a percentile grade means
  // anything; a full-time blocker never gets there. Say so on hover rather
  // than leaving a gap where every other player has a number.
  const ungraded = r == null && !player.ol
  const ungradedTitle = ungraded
    ? `${player.player_name} — not graded (${player.snaps ?? 0} snaps, no graded plays)`
    : player.player_name
  return (
    <button type="button" className={`player ${player.ol ? 'ol' : ''}`} style={{ left: `${player.x}%`, top: `${top}%` }} title={ungradedTitle} onClick={() => onSelect(player)}>
      <span className="pav" style={{ background: teamPrimaryColor(player.team) }}>
        ...
        {ungraded && <span className="badge b-none" aria-label="not graded">–</span>}
      </span>
