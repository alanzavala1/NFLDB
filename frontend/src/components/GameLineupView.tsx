...
// Why a player carries no rating. Two causes look identical on the field but
// are different facts: nothing countable happened to him at all, or something
// did and there was too little of it to sit on a percentile curve. Linemen are
// excluded — they intentionally carry no individual grade, only the unit one.
function ungradedReason(p: LineupPlayer): string | null {
  if (p.rating != null || isOL(p)) return null
  const snaps = p.snaps ?? 0
  const n = p.plays_counted ?? 0
  if (n > 0) return `${snaps} snaps, ${n} graded play${n === 1 ? '' : 's'}, too few to rate`
  return `${snaps} snaps, no graded plays`
}
...
function PlayerNode({ player, mirror, onSelect }: { player: Placed; mirror: boolean; onSelect: (p: LineupPlayer) => void }) {
  ...
  const reason = ungradedReason(player)
  const ungradedTitle = reason ? `${player.player_name} — not graded (${reason})` : player.player_name
  return (
    <button type="button" className={`player ${player.ol ? 'ol' : ''}`} style={{ left: `${player.x}%`, top: `${top}%` }} title={ungradedTitle} onClick={() => onSelect(player)}>
      <span className="pav" style={{ background: teamPrimaryColor(player.team) }}>
        ...
        {/* Rendering nothing made "not enough to grade" look identical to
            a data error. The dash is the honest answer. */}
        {reason && <span className="badge b-none" aria-label="not graded">–</span>}
      </span>
  ...
  return <div className="rot-col">{team.rotation.slice(0, 5).map(p => {
    ...
    return (
      <div className="rrow" key={`${team.team}-${p.player_id ?? p.pfr_player_id}`}>
        ...
        {/* The rail sorts rated players first, so a blank row here reads as a
            hole in the data rather than the bottom of the list. */}
        {reason && <span className="rbadge b-none" title={`Not graded — ${reason}`} aria-label="not graded">–</span>}
      </div>
    )
  })}</div>
}
...
function Popup({ game, player, onClose }: { game: GameDetail; player: LineupPlayer | null; onClose: () => void }) {
  ...
  const ungraded = ungradedReason(player)
  return <div className="lineup-overlay" onClick={onClose}><div className="lineup-modal" onClick={e => e.stopPropagation()}>
    <div className="modal-head"><div className="modal-avatar" style={{ background: teamPrimaryColor(player.team) }}>{player.headshot_url ? <img src={player.headshot_url} alt="" /> : initials(player.player_name)}</div><div><div className="modal-name">{player.player_name}</div><div className="modal-sub">{player.position} · {player.team} · {gameLabel(game)} · {pct(player.snap_pct)} of snaps</div>{ungraded && <div className="modal-sub ungraded">Not graded — {ungraded}</div>}</div>{player.rating != null && <span className={`rbadge ${ratingClass(player.rating)} modal-rating`}>{player.rating.toFixed(1)}{player.rating >= 8.5 ? ' ✦' : ''}</span>}{ungraded && <span className="rbadge b-none modal-rating" title={`Not graded — ${ungraded}`}>–</span>}<button className="modal-close" onClick={onClose}>✕</button></div>
